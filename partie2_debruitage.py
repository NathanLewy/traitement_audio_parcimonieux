...
coeffs = pywt.wavedec(signal_bruite, wave, level=n_level)
...
for i, c in enumerate(coeffs_detail):
    c = pywt.threshold(np.abs(c), threshold, mode='hard')
...
#RSB
threshold_values = np.linspace(0.0001, 1, 50)
...
for i in range(len(threshold_values)):
    coeffs = pywt.wavedec(signal_bruite, wave, level=n_level)
    coeffs_thresholded = [pywt.threshold(c, threshold_values[i], mode='hard') for c in coeffs]
    signal_reduit = pywt.waverec(coeffs_thresholded, wave)
